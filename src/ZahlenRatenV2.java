...
public class ZahlenRatenV2 {
    ...
    private  static int lvl4 () {
        ...
        while (guess != secretNumber) {
            ...
            if (guess != secretNumber) {
                ...
                rand = randomNummer(guessList);
                System.out.println("\u001B[33m Probieren Sie bitte mit " + rand + " von \n" + guessList);
                if (isZug) {
                    guess = randomNummer(guessList);
    ...
    private static int randomNummer (ArrayList<Integer> zahlen) {
        Random r = new Random();
        int rand =0;
        for(int i = 0; i< zahlen.size(); i++) {
            int randomIndex = r.nextInt(zahlen.size());
            rand = zahlen.get(randomIndex);
        }
        return rand;
    }
}